...
static void GenerateALUCommand(std::ostream& stream, Op op, std::variant<uint8_t, int16_t> a, std::variant<uint8_t, int16_t> b, uint8_t resultAddress)
{
	...
	if (a.index() == 0) {
		streamWrite(stream, std::get<0>(a));
	}
	else if (a.index() == 1) {
		streamWrite(stream, std::get<1>(a));
	}

	if (b.index() == 0) {
		streamWrite(stream, std::get<0>(b));
	}
	else if (b.index() == 1) {
		streamWrite(stream, std::get<1>(b));
	}
...
void ByteCodeBuilder::NewCompare(std::variant<uint8_t, int16_t> a, std::variant<uint8_t, int16_t> b)
{
	...
	streamWrite(m_Code, command);
	if (a.index() == 0) {
		streamWrite(m_Code, std::get<0>(a));
	}
	else if (a.index() == 1) {
		streamWrite(m_Code, std::get<1>(a));
	}

	if (b.index() == 0) {
		streamWrite(m_Code, std::get<0>(b));
	}
	else if (b.index() == 1) {
		streamWrite(m_Code, std::get<1>(b));
	}
}
